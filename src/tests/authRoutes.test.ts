...
describe("Authentication Routes", () => {
  ...
  describe("Role-Based Access Control", () => {
    ...
    it("should log in successfully and create a session for admin", async () => {
      // First, sign up a user
      await request(app).post("/auth/signup").send({
        username: "adminuser",
        password: "password123",
    ...
    it("should allow access to restricted route with valid session", async () => {
      // Access the restricted route with the session cookie
      const response = await request(app).get("/admin").set("Cookie", cookie);

      expect(response.status).toBe(200); // Expect successful access
      expect(response.text).toContain("Welcome Admin"); // Check for correct response
    });
    ...
    it("should log out successfully and destroy the session", async () => {
      ...
      // Try to access the restricted route again with the same cookie
      const protectedResponse = await request(app)
        .get("/admin")
        .set("Cookie", cookie);

      expect(protectedResponse.status).toBe(403); // Expect Unauthorized due to session destruction
    });

    it("should log in successfully and create a session for simple user", async () => {
      // First, sign up a user
      await request(app).post("/auth/signup").send({
        username: "simpleuser",
        password: "password123",
    ...
    it("should allow access to user route with valid session", async () => {
      // Access the user route with the session cookie
      const response = await request(app).get("/user").set("Cookie", cookie);

      expect(response.status).toBe(200); // Expect successful access
      expect(response.text).toContain("Welcome User"); // Check for correct response
    });

    it("should not allow access to restricted route with valid session", async () => {
      // Access the restricted route with the session cookie
      const response = await request(app).get("/admin").set("Cookie", cookie);

      expect(response.status).toBe(403); // Expect successful access
      expect(response.text).toContain("Forbidden. Insufficient role"); // Check for correct response
    });
    ...
    it("should log out successfully and destroy the session", async () => {
      ...
      // Try to access the user route again with the same cookie
      const protectedResponse = await request(app)
        .get("/user")
        .set("Cookie", cookie);
